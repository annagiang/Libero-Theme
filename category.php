...
get_header(); ?>
	...
	<div id="primary" class="content-area">
		...
		<?php
			...
			$is_default = rs::getField('rst_cat_layout',$post_id);
			$layout = $is_default ? rs::getField('rst_cat_layout',$post_id) : ( get_theme_mod('libero_cat_layout') ? get_theme_mod('libero_cat_layout') : 1 );
			$is_sidebar = $layout == 2 || $layout == 3;
			$sidebar = $is_default ? rs::getField('rst_cat_sidebar',$post_id) : ( get_theme_mod('libero_cat_sidebar') ? get_theme_mod('libero_cat_sidebar') : 'sidebar-1');
			
			$is_template_default = rs::getField('libero_cat_template',$post_id);
			$template = $is_template_default ? rs::getField('libero_cat_template',$post_id) : ( get_theme_mod('libero_cat_template') ? get_theme_mod('libero_cat_template') : 'large' );
			
			$numberpost = $is_template_default ? rs::getField('libero_cat_numberpost',$post_id) : ( get_theme_mod('libero_cat_numberpost') ? get_theme_mod('libero_cat_numberpost') : 10 );
			$excerpt_length = $is_template_default ? rs::getField('libero_cat_excerpt_length',$post_id) : ( get_theme_mod('libero_cat_excerpt_length') ? get_theme_mod('libero_cat_excerpt_length') : 30 );
			$pagenavi = $is_template_default ? rs::getField('libero_cat_pagenavi',$post_id) : ( get_theme_mod('libero_cat_pagenavi') ? get_theme_mod('libero_cat_pagenavi') : 'number' );
			$number_loadmore = $is_template_default ? rs::getField('libero_cat_number_loadmore',$post_id) : ( get_theme_mod('libero_cat_number_loadmore') ? get_theme_mod('libero_cat_number_loadmore') : 4 );
			$columns = $is_template_default ? rs::getField('libero_cat_columns',$post_id) : ( get_theme_mod('libero_cat_columns') ? get_theme_mod('libero_cat_columns') : 2 );
