...
(function ($) {
    $.fn.rsSelectBox = function (options, value) {
        ...
        options = $.extend({            
            ...
            itemTemplate: function(html, value, attr){
				if( value != '' )
					return '<span '+ attr +' class="rs-select-item">' + html + '</span>';
				else
					return '<span '+ attr +' class="rs-select-item hidden">' + html + '</span>';
            },
            afterInit: false,
            afterShow: false,
            beforeShow: false
        ...
        this.filter('select').each(function () {
            ...
            slto.addClass('rs-select-hidden').find('option').each(function () {
				var object = $(this).attr();
				delete object.class;
				var attr = '';
				$.each(object, function(key, value){
					attr += key +'="'+ object[key] +'" ';
				});
				if( object["data-color"] != undefined ) {
					var html = '<span style="background-color: '+ object["data-color"] +';" class="rst-color"></span>';
				} else {
					var html = options.itemHtmlFilter($(this).text(), $(this).val());
				}
                var temp = options.itemTemplate(html, $(this).val(), attr);
                opts.append(temp);
            });
