	...
	rs.controls.upload = {
		show: function(options){
			...
			if(wp.media){
				...
				tb_show(options.title, rs.wordpress.admin_url + '/media-upload.php?tab=library&post_id=0&type=' + options.type + '&multiple=' + options.multiple +'&buttontext=' + options.insertText +'&rsupload=tb&TB_iframe=true');
	...
	$('#media-upload td.savesend input').val($.parseParams(location.search.substring(1)).buttontext).click(function(event){
		...
		if(win.rs.controls.upload.tb_send){
			...
			var data = {
				...
				icon: image ? rs.wordpress.home_url + "/wp-includes/images/crystal/default.png" : table.find('thead .thumbnail').attr('src'),
				filesize: 'undefined',
				fizename: fizename,
				mime: mime
			};
